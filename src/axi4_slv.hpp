...
class axi4_slv { // class axi4_slave variant for axi4_ptr_t
public:
    axi4_slv() {
    }
    void reset() {
...
private:
    ...
    w_packet cur_w;
    int r_index = -1;
    void input_transaction(axi4_ptr_t &pin) {
        // input ar
        if (*pin.arvalid && *pin.arready) { // ar.fire
            ar_packet tmp;
            tmp.id = pin.get_arid();
            tmp.addr = pin.get_araddr();
            tmp.len = *pin.arlen;
            tmp.size = *pin.arsize;
            tmp.d_width = pin.d_width;
        ...
        if (*pin.wvalid && *pin.wready) { // w.fire
            cur_w.d_width = pin.d_width;
            for (int i = 0; i < pin.get_dwidth_bits() / 8; i++) {
    ...
    void output_transaction(axi4_ptr_t &pin) {
        // output r {
